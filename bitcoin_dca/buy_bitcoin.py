...
if GMAIL_USER_NAME is not None:
    email_notification = EmailNotification(
        GMAIL_USER_NAME, GMAIL_PASSWORD, EMAIL_NOTICE_RECEIVER
    )
...
while True:
    ...
    try:
        ...
        if (
            ...
            coinbase_pro.withdrawBitcoin(
                coinbase_pro.btcAccount().balance, address_selector.getWithdrawAddress()
            )
    ...
    Logger.info(
        f"Waiting until {next_buy_datetime.strftime('%Y-%m-%d %H:%M:%S')} "
